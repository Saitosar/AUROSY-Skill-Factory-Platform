...
- Экспорт keyframes в Авторинг с теми же строковыми именами суставов Phase 0, что и при работе с WebSocket-телеметрией.
...
Файл [`jointMapping.ts`](../web/frontend/src/mujoco/jointMapping.ts) определяет соответствие индексов `qpos` MuJoCo и строковых имён Phase 0 (`JOINT_MAP` на бэкенде). Экспорт keyframes в Авторинг использует эти же имена, чтобы `POST /api/validate` оставался согласованным.
